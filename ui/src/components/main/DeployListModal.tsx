...
export const DeployListModal: React.FC<{
    ...
    React.useEffect(() => {
        ...
        const handleClickOutside = (e: MouseEvent) => {
            if (searchPaneRef.current && !searchPaneRef.current.contains(e.target as Node)) {
                setSearchKeyword('');
    ...
    const handleAdd = (fileToAdd: string) => {
        ...
        actions.deploy.setStateSearchResult(state.deploy.searchResult.filter(item => item !== fileToAdd));
    };
    ...
    const handleRemove = (type: 'java' | 'query', fileToRemove: string) => {
        ...
        actions.deploy.updateDeployFiles(newDeployFiles, fileToRemove, type, 'remove');
    };

    // 참조 클래스 자동 추가 핸들러
    const handleAnalyzeReferenceChain = () => {
        const javaFiles = state.deploy.deployFileList.java;
        if (javaFiles.length === 0 || isAnalyzing) { return; }
        setIsAnalyzing(true);
    ...
    return (
        ...
        >
            <div className="deploy-list-layout" style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                {/* 검색 도구 영역 */}
                <div ref={searchPaneRef} className="search-pane" style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                        ...
                        {searchKeyword.trim().length > 0 && (
                            <div style={{
                                ...
                                {state.deploy.searchResult.length === 0 ? (
                                    <div style={{ padding: '8px', color: 'var(--vscode-descriptionForeground)', fontSize: '11px', textAlign: 'center' }}>
                                        검색 결과가 없습니다.
                                    </div>
                                ) : (
                                    state.deploy.searchResult.map((file, idx) => (
                                        !state.tomcat.running && <div key={idx} className="tree-item" title={getTooltip(file)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 8px', cursor: 'pointer', borderBottom: '1px solid var(--vscode-panel-border)' }} onClick={() => handleAdd(file)}>
                                            <span style={{ fontSize: '12px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                                {getFileName(file)}
                                                <span style={{ marginLeft: '8px', fontSize: '10px', color: 'var(--vscode-descriptionForeground)' }}>{file.includes('/src/java/') ? '(Java)' : '(Query)'}</span>
                                            </span>
                                            <button className="icon-btn tree-item-action" style={{ width: '20px', height: '20px', border: 'none', background: 'transparent', color: 'inherit', padding: 0 }}>+</button>
                                        </div>
                                    ))
                                )}
                            </div>
                ...
                <div className="list-pane" style={{ display: 'flex', flexDirection: 'column' }}>
                        ...
                        {!state.tomcat.running && (
                            <div style={{ display: 'flex', gap: '6px', marginLeft: 'auto' }}>
                                <button
                                    ...
                                    disabled={state.deploy.deployFileList.java.length === 0 || isAnalyzing}
                                    onClick={handleAnalyzeReferenceChain}
                                    style={{
                                        fontSize: '11px',
                                        padding: '3px 8px',
                                        cursor: state.deploy.deployFileList.java.length === 0 || isAnalyzing ? 'not-allowed' : 'pointer',
                                        opacity: state.deploy.deployFileList.java.length === 0 || isAnalyzing ? 0.5 : 1,
                    ...
                    <div className="tree-view-container" style={{
                        ...
                        <div className="tree-section">
                            ...
                            >
                                <span style={{ transform: isJavaOpen ? 'rotate(90deg)' : 'rotate(0deg)', transition: 'transform 0.1s', display: 'inline-block', marginRight: '4px', fontSize: '10px' }}>▶</span>
                                Java ({state.deploy.deployFileList.java.length})
                            </div>
                            {isJavaOpen && (
                                <div className="tree-content">
                                    {state.deploy.deployFileList.java.length === 0 ? (
                                        <div style={{ padding: '4px 8px 4px 24px', fontStyle: 'italic', color: 'var(--vscode-descriptionForeground)' }}>선택된 파일이 없습니다.</div>
                                    ) : (
                                        state.deploy.deployFileList.java.map((file, idx) => (
                                            <div key={idx} className="tree-item" title={getTooltip(file)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '4px 8px 4px 24px', cursor: 'pointer' }}>
                                                <span style={{ fontSize: '13px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{getFileName(file)}</span>
                                                {!state.tomcat.running && <button className="icon-btn tree-item-action" onClick={(e) => { e.stopPropagation(); handleRemove('java', file); }} style={{ width: '20px', height: '20px', border: 'none', background: 'transparent', color: 'inherit', padding: 0 }}>✕</button>}
                                            </div>
                                        ))
                                    )}
                                </div>
                        ...
                        <div className="tree-section">
                            <div
                                ...
                                style={{ display: 'flex', alignItems: 'center', padding: '4px 8px', cursor: 'pointer', fontWeight: 'bold', fontSize: '11px', textTransform: 'uppercase' }}
                            >
                                <span style={{ transform: isQueryOpen ? 'rotate(90deg)' : 'rotate(0deg)', transition: 'transform 0.1s', display: 'inline-block', marginRight: '4px', fontSize: '10px' }}>▶</span>
                                ...
                                <div className="tree-content">
                                    {state.deploy.deployFileList.query.length === 0 ? (
                                        ...
                                        state.deploy.deployFileList.query.map((file, idx) => (
                                            <div key={idx} className="tree-item" title={getTooltip(file)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '4px 8px 4px 24px', cursor: 'pointer' }}>
                                                <span style={{ fontSize: '13px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{getFileName(file)}</span>
                                                {!state.tomcat.running && <button className="icon-btn tree-item-action" onClick={(e) => { e.stopPropagation(); handleRemove('query', file); }} style={{ width: '20px', height: '20px', border: 'none', background: 'transparent', color: 'inherit', padding: 0 }}>✕</button>}
                                            </div>
                                        ))
                                    )}
                                </div>
